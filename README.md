...
The versioned, unknown-field-rejecting metadata schema and examples are documented in the [Source Repository guide](docs/source-guide.md), [ADR 0004](docs/adr/0004-narrow-patch-metadata.md), and [ADR 0013](docs/adr/0013-declared-patch-series.md). Declared series metadata controls only stable identity, mutable display text, and member order; the existing `patches` overlay can describe implicit series and exact Pi Base compatibility. Invalid metadata cannot suppress convention-discovered Patches. Metadata cannot add dependencies, hooks, scripts, cross-series ordering, build recipes, verifiers, Artifact Sets, or solver behavior.
