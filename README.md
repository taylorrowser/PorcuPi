...
PorcuPi also recursively discovers tracked regular `*.patch` files beneath the Source Repository's root `patches/`. Every standalone Patch File becomes an implicit one-file Patch Series whose stable source identity is its full structural path. PorcuPi rejects symbolic links, Git submodules, unsafe paths, non-regular files, and boundary escapes, and saves the series identity plus its exact member commit, path, and SHA-256. Patch Series appear beside Pi resources in add and manage, but never receive an Installation Scope or enter Pi package settings. Saving Patch intent reports it as pending and never rebuilds or changes the active Managed Pi Composition.
...
Apply previews the complete deterministic flattening by canonical Source Repository locator and Patch Series structural identity, including each exact member commit, path, and digest, then requires Enter or Space confirmation. Escape or Ctrl-C cancels without changing activation.
