...
The versioned, unknown-field-rejecting metadata schema and examples are documented in the [Source Repository guide](docs/source-guide.md), [ADR 0004](docs/adr/0004-narrow-patch-metadata.md), [ADR 0013](docs/adr/0013-declared-patch-series.md), and [ADR 0014](docs/adr/0014-artifact-pi-base-compatibility.md). A source may declare an exact Pi Base compatibility default and complete per-Artifact overrides for Patch Series, Extensions, Skills, Prompts, and Themes. Mismatches remain visible but cannot be selected; matches never bypass PorcuPi's fixed verification. Invalid metadata cannot suppress convention discovery. Metadata cannot add commands, dependencies, hooks, scripts, cross-series ordering, build recipes, force behavior, custom verifiers, activation policy, Artifact Sets, or solver behavior.
