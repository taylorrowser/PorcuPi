...
Every interactive Managed Pi starts with one fixed PorcuPi release-status row. Online startup checks the official npm release asynchronously and asks a newer exact target package to re-evaluate cached target Pi Base and checker-contract evidence before running any needed non-mutating Upgrade Readiness Check at reduced process priority. Matching cached ready or blocked evidence appears immediately on the next launch; changed bound inputs make it stale. Pi's `--offline` mode suppresses both startup activities, while `PORCUPI_BACKGROUND_READINESS=0` suppresses only background assessment and retains cached evidence. The row remains informational, stays out of conversation history, and never installs or activates an update. When an exact newer release is available, run the displayed `npx --yes porcupi@<version>` command in another terminal after leaving the current Managed Pi session; the guided installer revalidates readiness before mutation.
...
PorcuPi also recursively discovers tracked regular `*.patch` files beneath the Source Repository's root `patches/`. A strict root `porcupi.json` can declare stable Patch Series Artifacts, each with an explicitly ordered list of one or more Patch Files. Every unclaimed standalone Patch File becomes an implicit one-file Patch Series whose stable source identity is its full structural path. PorcuPi rejects duplicate claims, symbolic links, Git submodules, unsafe or missing paths, non-regular files, and boundary escapes, and saves each series identity plus its complete ordered member commit, path, and SHA-256 inventory. Patch Series appear beside Pi resources in add and manage, but never receive an Installation Scope or enter Pi package settings. Saving Patch intent reports it as pending and never rebuilds or changes the active Managed Pi Composition.

The versioned, unknown-field-rejecting metadata schema and examples are documented in the [Source Repository guide](docs/source-guide.md), [ADR 0004](docs/adr/0004-narrow-patch-metadata.md), [ADR 0013](docs/adr/0013-declared-patch-series.md), [ADR 0014](docs/adr/0014-artifact-pi-base-compatibility.md), and [ADR 0016](docs/adr/0016-structural-tracked-branch-inventories.md). A source may declare an exact Pi Base compatibility default, complete per-Artifact overrides, and data-only structural content roots for already discovered resources. Mismatches remain visible but cannot be selected; matches never bypass PorcuPi's fixed verification. Invalid metadata cannot suppress convention discovery. Metadata cannot add commands, dependencies, hooks, scripts, cross-series ordering, build recipes, force behavior, custom verifiers, activation policy, Artifact Sets, or solver behavior.
...
Apply previews the complete deterministic flattening by canonical Source Repository locator and stable Patch Series identity, preserving declared member order and showing each exact member commit, path, and digest, then requires Enter or Space confirmation. Escape or Ctrl-C cancels without changing activation.
