...
Version-aware Release Installation compares its exact invoking package version with the receipt-verified installed runtime version before showing an installation prompt:
...
The first migration contract is `PorcuPi 0.1.0 / state schema 1` to `PorcuPi 0.2.0 / state schema 1`. It applies only to an intact zero-Patch installation with empty Selection Intent. Selected Artifact migration belongs to issue #43, and publication-boundary interruption, corruption, and lease hardening belong to issue #44. A future source version or schema requires another explicit migration contract; a numerically newer installer alone is insufficient.
