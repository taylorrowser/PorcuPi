...
The stage strictly records both source and target Activations and exact source and target runtime inventory receipts. Recovery validates those closed records, both staged trees, every source-or-target intermediate runtime entry, the target Composition, and a freshly reconstructed runtime from the invoking exact release. It continues a pre-activation transaction only while current Activation still exactly equals the recorded source. If a later serialized lifecycle operation changed Activation, recovery never overwrites it: a pre-commit stage restores the historical runtime and is abandoned, while a stage whose target Activation was already committed retains the target runtime and discards only obsolete recovery evidence.

The target runtime accepts historical runtime-receipt schema 1 for upgrade preflight and writes schema 2 after publication. Schema 2 is also a compatibility fence: the immutable v0.1.0 installer rejects a successfully upgraded installation during its strict runtime verification, before it can reconcile command ownership. The v0.2.0 installer verifies the target runtime and unchanged launchers, then atomically replaces Activation. The new active entry names the target zero-Patch Composition and `previous` names the former active Composition. Both runtimes read state schema 1, so any compatible file-publication intermediate and the runtime-first switch can read the old Activation. Activation replacement is the transaction commit point: any later bookkeeping or cleanup error is reported as committed success with the exact owner stage retained for retry, never as an upgrade failure with the target already authoritative. Successful cleanup retains only the active and one previous Composition under ADR 0007.
