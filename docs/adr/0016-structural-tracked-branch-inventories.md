...
For a selected Pi resource, the fingerprint also compares only the Source Repository root package manifest's exact dependency fields, exact npm install-lifecycle script declarations, and applicable committed root npm lock bytes. That root manifest is the package manifest applicable to Pi's Git-package installation; a nested `package.json#pi` may participate in conventional Extension discovery but does not replace the root package's shared installation inputs. PorcuPi does not parse their meaning or trace imports, scripts, workspaces, dependency graphs, build inputs, or runtime file access. Conservative noise from these bounded shared inputs is accepted.
