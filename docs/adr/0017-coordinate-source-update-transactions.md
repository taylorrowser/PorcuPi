...
For selected Pi resources, one source acceptance reconciles every affected global and project Installation Scope through Pi's public package lifecycle. PorcuPi stages exact narrowed settings, never supplies project approval, and saves Selection Intent only after all package operations and settings verification succeed. Failure restores every prior settings snapshot and compensates every attempted package operation because Pi can change a checkout before reporting a later dependency failure. PorcuPi asks Pi to remove attempted new-scope packages and reinstall every prior exact package source, drains all scopes even when one compensation fails, and restores the exact settings bytes again afterward. A failed removal can leave only unconfigured Pi-owned package data where no prior source snapshot existed. Prior-source reinstallation retries with bounded backoff until every exact snapshot is restored; the failed source transaction cannot return or save Selection Intent before that compensation completes. Project trust remains Pi-owned; denial commits no source snapshot.

The complete `porcupi add` and `porcupi manage` operations, including source input, candidate resolution, guided review, final revalidation, package reconciliation, and Selection Intent save, hold ADR 0007's shared lifecycle lock. They therefore serialize source mutation with each other and with Release Installation and upgrade recovery, apply, rollback, `pi` ownership, Composition cleanup, and uninstall.
