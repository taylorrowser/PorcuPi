...
Each uploaded JSON and Markdown report records the exact npm package name/version and packed integrity, repository revision and tag, Pi Base, historical and Stock Pi fixtures, platform, every public command, outcome, and duration. Candidate tests use a local tarball produced by `npm pack`; they do not publish mutable test versions. Maintainers complete the [release checklist](releases/release-checklist.md) before publication.
