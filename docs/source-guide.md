...
For each selected Pi resource, bounded shared package inputs are the applicable package manifest's exact dependency fields, exact install-lifecycle script declarations, and applicable committed npm lock as a whole. PorcuPi compares these conservatively but does not parse scripts, imports, lock dependency graphs, workspaces, build inputs, or runtime filesystem access. Documentation, tests, unrelated files, and new independent unselected Artifacts do not by themselves define an update for an existing selection. An undeclared helper outside an Artifact boundary can therefore be missed: colocate it, declare it in `content`, or tell users to use the explicit latest-commit review action.
