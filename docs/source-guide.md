...
Publish each candidate as an immutable commit, advance the branch by fast-forward, and keep every selected Artifact from the Source Repository coherent at that one commit. At each online Managed Pi startup, PorcuPi checks retained Tracked Branch heads asynchronously with bounded concurrency and quiet failure, then caches only relevant compatible selected-content candidates for the fixed status row and network-free `porcupi status` expansion. Offline startup retains matching cached source status without contacting remotes. Pinned tags/commits and migrated exact snapshots without proven branch identity are never checked. Branch movement alone never mutates Selection Intent, Pi settings, pending Patch intent, or Managed Pi Activation. Adoption requires review of one resolved exact candidate. `porcupi manage` offers a structurally changed candidate for guided review. When several Source Repositories advance, manage shows their changed, unchanged, or blocked status and lets the user choose one reviewable source; cancellation or a blocker for one source does not mutate or hide the others. When the latest commit has an unchanged selected structural inventory, its explicit review action still permits guided adoption. Final acceptance re-resolves the branch and requires the exact reviewed commit and inventory before any mutation. A later accepted commit replaces that source's older pending snapshot rather than queuing deltas; each other source's latest accepted desired snapshot remains intact in the aggregate Selection Intent.

A source acceptance reconciles all of that source's selected global and project Pi resources as one package transaction before saving intent. Package failure or project trust denial restores prior settings and commits no candidate snapshot. Pi alone decides project trust. Source review and reconciliation hold PorcuPi's shared lifecycle lock, and Patch changes never alter active Managed Pi until a later explicit `porcupi apply`. A deleted branch, changed channel, ambiguous ref, or non-fast-forward rewrite is refused while the accepted snapshot remains authoritative.

A Post-release Compatibility Update follows the same rule: commit the selected content or exact compatibility metadata that supports the new Pi Base, test that exact commit, and fast-forward the retained branch. This publishes a candidate for explicit compatibility assessment and review; it does not make a compatibility declaration proof, mutate followers, or activate an update automatically. A failed source check can resolve to an actionable Inter-release candidate on a later startup. Candidate notices remain advisory and separate from guided source-update adoption and Tracked Branch retention; recognition against a different target PorcuPi release is separate Post-release Compatibility work.
...
- [Coordinated source update transaction decision](adr/0017-coordinate-source-update-transactions.md)
- [Tracked Branch availability cache decision](adr/0019-cache-tracked-branch-availability.md)
